...
export const handler = async (event, context) => {
    try {
        ...
        const { audioData, timepoints } = await synthesizeWithEchogram(body.narrative_sections, googleApiKey, googleVoice, language_code, body.speaking_rate);
        console.log("google synthesize done, here timepoints", timepoints);
        const wordsTimepoints = await alignAudioWithTranscript(audioData, body.narrative_sections.join(' '), language_code);
        console.log("echogarden done, here are words", wordsTimepoints);
        const metadata = {
            marked_timepoints: timepoints,
            words: wordsTimepoints,
        };
        const { audioUrl, timelineUrl } = await uploadContentToS3(audioData, metadata, S3_BUCKET_NAME, body.user_id, body.script_titile.toLowerCase());
