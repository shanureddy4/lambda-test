...
async function uploadContentToS3(audio: Uint8Array, timeline: object, bucketName: string, user_id:string, script_titile:string) {
    const uid = uuidv4();
...
async function getParameters(voice: string) {
    ...
    try {
        ...
        console.error("Error fetching parameters:", (error as Error).message);
        throw error;
    }
}
export function getLangaugeCode(googleVoice:string):string{
